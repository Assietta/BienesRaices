  ...
  try {
    if (payment.type === "payment") {
      ...
      const newOrder = await Order.create({
        ...
        statement_descriptor_usernameMP: String(
          data?.response.statement_descriptor
        ),
        ...
        transaction_amount_refunded: parseFloat(
...
const approved = async (req, res) => {
  axios
    .get(`https://api.mercadopago.com/v1/payments/${payment["data.id"]}}`, {
      headers: {
